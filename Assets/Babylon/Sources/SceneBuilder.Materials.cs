...
namespace Unity3D2Babylon
{
    partial class SceneBuilder
    {
        ...
        private BabylonMaterial DumpMaterial(Material material, bool receiveShadows, int lightmapIndex = -1, Vector4 lightmapScaleOffset = default(Vector4), int lightmapCoordIndex = -1)
        {
            ...
            else if (material.shader.name.StartsWith("Nature/", StringComparison.OrdinalIgnoreCase))
            {
                return DumpPBRMaterial(material, receiveShadows, lightmapIndex, lightmapScaleOffset, lightmapCoordIndex, true);
            }
        ...
        private void DumpGlossinessReflectivity(Material material, bool metallic, float microSurfaceScale, float highlightIntensity, BabylonSystemMaterial babylonPbrMaterial)
        {
            ...
            // ..
            float metalness = 0.0f, glossiness = 0.0f;
        ...
        private static void DumpTransparency(Texture2D texture, Material material, BabylonStandardMaterial babylonStdMaterial)
        {
			...
			Unity3D2Babylon.Tools.AlphaMode alphaMode = Unity3D2Babylon.Tools.AlphaMode.OPAQUE;
			...
			string materialRenderType = material.GetTag("RenderType", false, "Opaque");
			if (hasCutoff == true && material.shader.name.IndexOf("Nature/SpeedTree") >= 0)
			{
               	// Note: Speed Tree Alpha Modes
				alphaMode = Unity3D2Babylon.Tools.AlphaMode.MASK;
			}
            else if (!String.IsNullOrEmpty(materialRenderType))
            {
				// Note: Transparent Shader Alpha Modes
                if (hasCutoff == true && (materialRenderType.IndexOf("TreeBark", StringComparison.OrdinalIgnoreCase) >= 0) || materialRenderType.IndexOf("TreeLeaf", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    alphaMode = Unity3D2Babylon.Tools.AlphaMode.MASK;
                }
                else if (materialRenderType.IndexOf("TransparentCutout", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    alphaMode = Unity3D2Babylon.Tools.AlphaMode.MASK;
                }
                else if (materialRenderType.IndexOf("Transparent", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    alphaMode = Unity3D2Babylon.Tools.AlphaMode.BLEND;
                }
            }
            babylonStdMaterial.alpha = babylonStdMaterial.diffuse[3];
            babylonStdMaterial.alphaCutoff = alphaCutoff;
            switch (alphaMode) {   
                case Unity3D2Babylon.Tools.AlphaMode.OPAQUE:
                    babylonStdMaterial.alpha = 1.0f;
                    break;
                case Unity3D2Babylon.Tools.AlphaMode.MASK:
                    if (babylonStdMaterial.diffuseTexture != null) {
                        babylonStdMaterial.diffuseTexture.hasAlpha = true;
                    }
                    break;
                case Unity3D2Babylon.Tools.AlphaMode.BLEND:
                    if (babylonStdMaterial.diffuseTexture != null) {
                        babylonStdMaterial.diffuseTexture.hasAlpha = true;
        ...
        private static void DumpTransparency(Texture2D texture, Material material, BabylonSystemMaterial babylonPbrMaterial)
        {
            ...
            babylonPbrMaterial.alphaCutoff = alphaCutoff;
            switch (alphaMode) {   
            case Unity3D2Babylon.Tools.AlphaMode.OPAQUE:
                babylonPbrMaterial.transparencyMode = 0;
                break;
            case Unity3D2Babylon.Tools.AlphaMode.MASK:
                babylonPbrMaterial.transparencyMode = 1;
                if (babylonPbrMaterial.albedoTexture != null) {
                    babylonPbrMaterial.albedoTexture.hasAlpha = true;
                }
                break;
            case Unity3D2Babylon.Tools.AlphaMode.BLEND:
                babylonPbrMaterial.transparencyMode = 2;
                if (babylonPbrMaterial.albedoTexture != null) {
                    babylonPbrMaterial.albedoTexture.hasAlpha = true;
