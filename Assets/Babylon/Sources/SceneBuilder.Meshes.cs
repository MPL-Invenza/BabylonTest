    ...
    partial class SceneBuilder
    {
        ...
        private BabylonMesh ConvertUnityMeshToBabylon(Mesh mesh, Transform transform, GameObject gameObject, float progress, ref UnityMetaData metaData, ref List<UnityFlareSystem> lensFlares, ref string componentTags, BabylonMesh collisionMesh = null, Collider collider = null)
        {
            ...
            if (mesh != null)
            {
                ...
                if (mesh.boneWeights.Length == mesh.vertexCount)
                {
                    ...
                    if (skin != null && skin.enabled && skin.bones != null && skin.bones.Length > 0)
                    {
                        UnityEngine.Debug.Log("Normalizing Bone Weights For: " + skin.name);
                        int[] indices = babylonMesh.matricesIndices;
                        float[] weights = babylonMesh.matricesWeights;
                        Tools.NormalizeMatricesWeights(skin, babylonMesh.numBoneInfluencers, ref indices, ref weights);
                        babylonMesh.matricesIndices = indices;
                        babylonMesh.matricesWeights = weights;
                    }
                    */
                }
                index = 0;
                ...
                if (terrain.isActiveAndEnabled)
                {
                    ...
                    if (terrainMeshes != null && terrainMeshes.Length > 0) {
                        ...
                        foreach (var terrainMesh in terrainMeshes)
                        {
                            ...
                            Tools.GenerateBabylonMeshData(terrainMesh, groundMesh);
                    ...
                    if (terrainColliders > 1) {
                        ...
                        if (collisionMeshes != null && collisionMeshes.Length > 0) {
                            ...
                            foreach (var collisionMesh in collisionMeshes)
                            {
                                ...
                                Tools.GenerateBabylonMeshData(collisionMesh, groundCollider);
        ...
        private BabylonTerrainSplat[] GetTerrainTextureAtlasInfo(Terrain terrain, bool bump)
        {
            ...
            if (terrain.terrainData.terrainLayers != null && terrain.terrainData.terrainLayers.Length > 0) {
                foreach (var splat in terrain.terrainData.terrainLayers) {
                    Vector2 tileSize = new Vector2(0, 0);
                    Vector2 tileOffset = new Vector2(0, 0);
                    Texture2D splatTexture = null;
                    if (bump) {
                        if (splat.normalMapTexture != null) {
                            string bumpTexturePath = AssetDatabase.GetAssetPath(splat.normalMapTexture);
                            ...
                            try
                            {
                                ...
                                splatTexture = splat.normalMapTexture.Copy();
                            }
                            catch(Exception ex)
                            {
                        ...
                        if (splat.diffuseTexture != null) {
                            tileSize = splat.tileSize;
                            tileOffset = splat.tileOffset;
                            Texture2D tempTexture = splat.diffuseTexture;
