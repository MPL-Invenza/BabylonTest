...
namespace Unity3D2Babylon
{
    public static class Tools
    {
        ...
        public static int GetMaxBoneInfluencers()
        {
            int result = 4;
            // DEPRECIATED
            // if (QualitySettings.skinWeights == SkinWeights.OneBone) result = 1;
            // else if (QualitySettings.skinWeights == SkinWeights.TwoBones) result = 2;
            return result;
        }
        ...
        */
        ...
        {
            return GameObjectUtility.AreStaticEditorFlagsSet(source, StaticEditorFlags.ContributeGI);
        }
        ...
        */
        ...
        public static void NormalizeMatricesWeights(SkinnedMeshRenderer skin, int influencers, ref int[] matricesIndices, ref float[] matricesWeights)
        {
            const float epsilon = 1e-3f;
            int size = matricesWeights.Length;
            int noInfluenceBoneIndex = skin.bones.Length;
    
            for (int i = 0; i < size; i += 4) {
                float weight = 0.0f;
                int firstZeroWeight = -1;
                for (var j = 0; j < 4; j++) {
                    float w = matricesWeights[i + j];
                    weight += w;
                ...
                if (weight > epsilon) {
                    ...
                    if (firstZeroWeight <= 3) {
                        matricesWeights[i + firstZeroWeight] = (1.0f - weight);
        ...
        public static void GenerateBabylonMeshData(Mesh mesh, BabylonMesh babylonMesh, bool recalculateMesh = false)
        {
            if (mesh == null || babylonMesh == null) return;
            // Recalculate
            ...
            {
                babylonMesh.tangents = new float[mesh.vertexCount * 4];
                foreach (Vector4 tt in mesh.tangents)
                {
                    babylonMesh.tangents[index * 4] = tt.x;
                    babylonMesh.tangents[(index * 4) + 1] = tt.y;
                    babylonMesh.tangents[(index * 4) + 2] = tt.z;
                    babylonMesh.tangents[(index * 4) + 3] = tt.w;
                    index++;
                }
            }
            // Tex Coords
            ...
            {
                babylonMesh.colors = new float[mesh.vertexCount * 4];
                foreach (Color cc in mesh.colors)
                {
                    babylonMesh.colors[index * 4] = cc.r;
                    babylonMesh.colors[(index * 4) + 1] = cc.g;
                    babylonMesh.colors[(index * 4) + 2] = cc.b;
                    babylonMesh.colors[(index * 4) + 3] = 1.0f;
                    index++;
                }
            }
            // Triangle Indices
        ...
        public static void GenerateBabylonMeshDataOld(Mesh mesh, BabylonMesh babylonMesh, BabylonScene babylonScene = null, Transform transform = null)
        {
            int index = 0;
            babylonMesh.positions = new float[mesh.vertexCount * 3];
            foreach (Vector3 vv in mesh.vertices)
